{{ config(materialized="incremental", unique_key="src_productCode") }}

with
    batch_data as (select etl_batch_date, etl_batch_no from etlmetadata.batch_control),
    existing_data as (
        select dw_product_id, src_productcode, dw_create_timestamp from {{ this }}
    ),
    max_row_number as (
...
join {{ ref("productlines") }} as pl on st.productline = pl.productline
