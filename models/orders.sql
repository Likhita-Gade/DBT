{{ config(materialized="incremental", unique_key="src_orderNumber") }}

with
    batch_data as (select etl_batch_date, etl_batch_no from etlmetadata.batch_control),
    existing_data as (
        select dw_order_id, src_orderNumber, dw_create_timestamp from {{ this }}
    ),
    max_row_number as (
...
join {{ ref("customers") }} c on st.customernumber = c.src_customernumber
