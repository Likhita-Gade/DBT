...
with
    ...
    existing_data as (
        select dw_customer_id, src_customernumber, dw_create_timestamp from "dev"."devdw"."customers"
    ),
    max_row_number as (
        select max(dw_customer_id) as max_customer_id
        from "dev"."devdw"."customers"
    )
select
...
left join "dev"."devdw"."employees" em on st.salesrepemployeenumber = em.employeenumber
