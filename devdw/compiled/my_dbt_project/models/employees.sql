...
with
    ...
    existing_data as (
        select dw_employee_id, employeenumber, dw_create_timestamp from "dev"."devdw"."employees"
    ),
    max_row_number as (
        select max(dw_employee_id) as max_employee_id
        from "dev"."devdw"."employees"
    ),
    source_data as (
        select
            st.employeenumber,
        ...
        join "dev"."devdw"."offices" ofc on st.officecode = ofc.officecode
